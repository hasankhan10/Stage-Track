...
const groupColors: Record<string, { bg: string; border: string; badge: string; text: string; accent: string }> = {
    ...
    'Archived': {
        ...
        border: 'border-slate-500/30 hover:border-slate-500/70',
...
export default async function PipelinePage() {
    ...
    // Fetch user's workspace_id to scope all queries correctly
    ...
    // Fetch client counts per stage — scoped to this workspace
    ...
    const totalValue = clients?.reduce((sum, c) => sum + (c.deal_value || 0), 0) || 0
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="flex items-start justify-between mb-4">
                                            <div className={`h-10 w-10 rounded-lg flex items-center justify-center bg-background/80`}
                                                style={{ boxShadow: `0 0 0 1px ${colors.accent}30` }}>
                                                <Icon className={`h-5 w-5 ${colors.text}`} />
                                            </div>
                                            <span className="text-[10px] font-bold text-muted-foreground/50 bg-muted/50 px-1.5 py-0.5 rounded">
                                                #{stage.id}
                                            </span>
                                        </div>
                                        ...
                                        <p className="text-[11px] text-muted-foreground leading-snug mb-4 line-clamp-2">
                                            {stage.description}
                                        </p>
